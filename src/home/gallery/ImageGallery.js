...
import { Animated, BackHandler, Easing, PanResponder, Platform, StatusBar, StyleSheet, View } from "react-native";
...
class ImageGallery extends React.Component {
    ...
    shouldComponentUpdate(nextProps) {
        return !shallowEqualsIgnoreKeys(this.props, nextProps, ["animationMeasurements", "item"]);
    }
    ...
    render() {
        ...
        let isDragging = lifecycle === DRAG_IN_PROGRESS || lifecycle === CLOSING_ANIMATION_IN_PROGRESS;
    ...
    _animateSlideOut = (vy, dy, duration = 150) => {
        ...
        setTimeout(() => {
            Animated.timing(zoomInOutValue, zoomAnimationConfig).start(({ finished }) => {
                this.props.dispatch(Actions.updateImageGalleryLifecycle(CLOSED_AND_IDLE));
    ...
    _isAnimatedImageVisible() {
        return [CLOSED_AND_IDLE, OPEN_AND_IDLE].indexOf(this.props.lifecycle) === -1;
    }
